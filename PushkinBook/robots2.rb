...
loop do
    ...
    i = rand(0..9)                            # переменная, получающая случайное значение робота (индекса в массиве), куда пришлось попадание
    d = rand(0..100)                          # переменная, получающая случайное значения "ущерба" для робота

    puts i 
    puts d 

    if arr1[i] == 0                           # условие для определения промаха по роботу (индексу в массиве)
        puts "Промазали по индексу #{i}"
    elsif (arr1[i] - d) < 0 || d == arr1[i]   # условие для признания робота "уничтоженным"
        arr1[i] = 0
        puts "Чужой робот по индексу #{i} уничтожен "
    elsif arr1[i] > 0                         # условие для начисления "ущерба" роботу
        arr1[i] = arr1[i] - d                 # начисление "ущерба" роботу (индексу в массиве)
